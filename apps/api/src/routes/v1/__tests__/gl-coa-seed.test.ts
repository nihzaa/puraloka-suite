...
describe('CoA — seed idempoten & per-company', () => {
  ...
  it('badan usaha BARU mendapat bagan akun lengkap', async () => {
    ...
    try {
      ...
      await client.query(`DELETE FROM approval_steps WHERE chain_id IN (SELECT id FROM approval_chains WHERE company_id IN (SELECT id FROM companies WHERE id = $1))`, [co[0].id])
      await client.query(`DELETE FROM approval_chains WHERE company_id IN (SELECT id FROM companies WHERE id = $1)`, [co[0].id])
