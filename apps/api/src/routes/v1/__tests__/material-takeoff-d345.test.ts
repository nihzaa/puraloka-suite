...
beforeAll(async () => {
  ...
  const { rows: clnt } = await client.query(
    `INSERT INTO clients (contact_person, phone, created_by) VALUES ('[TEST-D345] Klien', '08', $1) RETURNING id`,
    [adminUserId])
  const { rows: pr } = await client.query(
    `INSERT INTO projects (client_id, pm_id, name, location, start_date, end_date, created_by)
     VALUES ($1, $2, '[TEST-D345] Proyek', 'Bandung', CURRENT_DATE, CURRENT_DATE + 30, $2) RETURNING id`,
