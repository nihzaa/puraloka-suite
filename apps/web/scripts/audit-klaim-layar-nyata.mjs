...
/*
  ...
  Versi sebelumnya memakai cwd, jadi penjaga ini hanya jalan bila dipanggil
  dari `apps/web`. Runner CI (`jalankan-semua-penjaga.mjs`) memanggilnya dari
  AKAR REPO — dan di sana ia mati dengan ENOENT `lib/peta-menu.ts`.

  Akibatnya bukan sekadar berisik: penjaga yang CRASH tidak menjaga apa pun,
  sementara laporannya terbaca sebagai "merah" yang menyatu dengan merah
  lain dan tak ada yang memeriksa sebabnya. Diukur 2026-08-27 — pesannya
  hanya "Node.js v24.16.0", tanpa satu pun temuan.
*/
const SKRIP = dirname(fileURLToPath(import.meta.url))
const AKAR = dirname(SKRIP)                       // apps/web
