// forked from design-sync lib/bundle.mjs — Next.js app components: stub next/navigation, next/link, next/image, next-themes to prevent scheduler bundling error
...
// Fork adds: next/navigation, next/link, next/image, next-themes stubs so
// Next.js app components don't pull react-dom internals (scheduler) into the IIFE.
export const reactShim = {
  ...
  setup(b) {
    ...
    // ── Next.js stubs ─────────────────────────────────────────────────────
    // These components are Next.js app components. Stub the framework imports
    // so they can be evaluated as isolated React components without a Next.js
    // runtime (and without pulling in react-dom internals via next/*).
    ...
    b.onResolve({ filter: /^next-themes$/ }, () => ({ path: 'next-themes-shim', namespace: 'shim' }));
...
exports.Fragment=R.Fragment;exports.Suspense=R.Suspense;exports.StrictMode=R.StrictMode;exports.Profiler=R.Profiler;`,
    ...
    b.onLoad({ filter: /^scheduler-shim$/, namespace: 'shim' }, () => ({
      // For Next.js app components, scheduler may be transitively imported.
      // Return a passthrough shim that defers to the scheduler React's vendor
      // bundle already loaded — avoids the throw while keeping concurrent mode.
      contents: `var S=(window.__dsVendorScheduler=window.__dsVendorScheduler||{});
var noop=function(){};var now=Date.now.bind(Date);
exports.unstable_scheduleCallback=S.unstable_scheduleCallback||function(p,c){setTimeout(c,0);return{};};
exports.unstable_cancelCallback=S.unstable_cancelCallback||noop;
exports.unstable_shouldYield=S.unstable_shouldYield||function(){return false;};
exports.unstable_requestPaint=S.unstable_requestPaint||noop;
exports.unstable_now=S.unstable_now||now;
exports.unstable_getCurrentPriorityLevel=S.unstable_getCurrentPriorityLevel||function(){return 3;};
exports.unstable_ImmediatePriority=1;exports.unstable_UserBlockingPriority=2;
exports.unstable_NormalPriority=3;exports.unstable_LowPriority=4;exports.unstable_IdlePriority=5;`,
      loader: 'js',
    }));

    // ── Next.js stub loaders ──────────────────────────────────────────────
    b.onLoad({ filter: /^next-nav-shim$/, namespace: 'shim' }, () => ({
      contents: `var noop=function(){};
exports.useRouter=function(){return{push:noop,back:noop,forward:noop,refresh:noop,replace:noop,prefetch:noop};};
exports.usePathname=function(){return'/';};
exports.useSearchParams=function(){return{get:noop,has:noop,getAll:noop,toString:function(){return'';}};};
exports.useParams=function(){return{};};
exports.redirect=noop;exports.notFound=noop;
exports.useSelectedLayoutSegment=function(){return null;};
exports.useSelectedLayoutSegments=function(){return[];};`,
      loader: 'js',
    }));
    b.onLoad({ filter: /^next-link-shim$/, namespace: 'shim' }, () => ({
      contents: `var R=window.React;
function Link(p){return R.createElement('a',{href:p.href,className:p.className},p.children);}
module.exports=Link;module.exports.default=Link;`,
      loader: 'js',
    }));
    b.onLoad({ filter: /^next-image-shim$/, namespace: 'shim' }, () => ({
      contents: `var R=window.React;
function Image(p){return R.createElement('img',{src:p.src,alt:p.alt||'',width:p.width,height:p.height,className:p.className});}
module.exports=Image;module.exports.default=Image;`,
      loader: 'js',
    }));
    b.onLoad({ filter: /^next-themes-shim$/, namespace: 'shim' }, () => ({
      contents: `var R=window.React;
exports.ThemeProvider=function(p){return R.createElement(R.Fragment,null,p.children);};
exports.useTheme=function(){return{theme:'light',setTheme:function(){},resolvedTheme:'light',systemTheme:'light',themes:['light','dark']};};`,
      loader: 'js',
    }));
  },
};
  ...
  try {
    buildResult = await build({
      ...
      define: {
        ...
        // NEXT_PUBLIC_* vars from .env.local — needed so Supabase client
        // initialises at module load time without throwing
        'process.env.NEXT_PUBLIC_SUPABASE_URL': '"https://tgozokxyvwmyvajgqfxw.supabase.co"',
        'process.env.NEXT_PUBLIC_SUPABASE_KEY': '"sb_publishable_6Z7IgDPCfjC_ZkOdlUkmuQ_iOYwo1mC"',
        'process.env.NEXT_PUBLIC_API_URL': '"http://localhost:3001"',
        'process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY': '"BCtMWjpUmaiZBRRSHvp19e0NjD-z7LwJj9f5u5xnthnL4hxVsi18SOE6CBigwFC6auq5lZkfEJbA9vhoRfk1bRA"',
      },
      // process polyfill — some bundled deps use process.env/browser/version
      // at module init time; a bare `define` doesn't cover `process` as a whole.
      banner: { js: 'var process=typeof process!=="undefined"?process:{env:{NODE_ENV:"development"},browser:true,version:"v18.0.0",platform:"browser",nextTick:function(f){setTimeout(f,0);}};' },
    });
  } catch (e) {
