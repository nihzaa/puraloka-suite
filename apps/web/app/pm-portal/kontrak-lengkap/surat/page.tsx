...
// ============================================================================
// Surat Masuk & Keluar — versi PM (Tahap 2, Task 14).
//
// Pakai endpoint LINTAS-PROYEK (`GET /api/v1/letters`) sebagai default — ini
// yang menjawab "surat mana yang wajib saya jawab hari ini" lintas SEMUA
// proyek PM (beda dari pola pemilih-proyek Task 12-13: endpoint ini SENGAJA
// dirancang lintas-proyek, lihat komentar `surat.ts`). Form "Surat Baru"
// tetap memilih SATU proyek karena `POST` per-proyek (`.../projects/:id/letters`).
//
// Endpoint:
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--gap-bagian)" }}>
      ...
      {!memuat && data && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 45%", padding: "var(--pad-kartu)", borderRadius: 12, background: "var(--surface)", border: "1px solid color-mix(in srgb, var(--border) 40%, transparent)", boxShadow: "var(--naik-1)" }}>
      ...
      {!memuat && data?.data.map((s) => (
        <div key={s.id} style={{ padding: "var(--pad-kartu-lega)", borderRadius: 16, background: "var(--surface)", border: "1px solid color-mix(in srgb, var(--border) 40%, transparent)", boxShadow: "var(--naik-1)", display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <BottomSheet terbuka={sheetTerbuka} onTutup={() => setSheetTerbuka(false)} judul="Surat Baru">
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <label htmlFor="arah-201" style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>
            Arah</label>
            <Pilihan id="arah-201" value={form.arah} onChange={(e) => setForm((f) => ({ ...f, arah: e.target.value as "masuk" | "keluar" }))}
              style={{ width: "100%", marginTop: 6, minHeight: 44, padding: "0 12px", borderRadius: 12, border: "1px solid var(--border)", fontSize: 14, background: "var(--surface)", color: "var(--text-primary)", boxSizing: "border-box" }}>
