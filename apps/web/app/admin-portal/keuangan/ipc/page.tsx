...
// ============================================================================
// Sertifikat IPC — Portal Admin/Direktur (Task 14, awal Tahap 3). Salinan
// APA ADANYA dari `pm-portal/keuangan/ipc/page.tsx` (Task 32 PM) — daftar per
// proyek + terbitkan + setujui.
//
// Gerbang `finance:view` (GET/list) dan `finance:invoice:create` (POST
// terbitkan, PATCH setujui) — admin+direktur SAMA-SAMA punya (diverifikasi
// `sertifikat-ipc.ts:36,85,125,229`). TIDAK PERLU penanganan 403 khusus
// seperti Dashboard/Piutang — halaman ini render normal untuk kedua role.
//
// TIDAK ADA tombol tolak/hapus — endpoint backend tak menyediakannya (hanya
// `PATCH .../setujui`). Draft yang salah dibiarkan sebagai draft (bisa
...
// MUAT (`galat` dari `useData`) — dua state berbagi satu tempat pernah
// menghapus pesan gagal-muat begitu aksi gagal.
//
// `daftarProyek` TIDAK memfilter `.filter((p) => p.pm)` — pola sama Task
// 7/9/10/11 (portal admin melihat SELURUH proyek, bukan hanya yang di-PM-i
// pengguna, beda dari pm-portal yang menyaring proyek milik PM yang login).
// ============================================================================
...
export default function AdminSertifikatIpcPage() {
          ...
          <div key={s.id} style={{ background: "var(--surface)", borderRadius: "var(--portal-radius-card)", padding: "var(--pad-kartu-lega)", border: "1px solid var(--border)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>{formatTanggal(s.tanggal)} · Progres diakui {s.progres_diakui_pct}%</div>
              </div>
              <span style={{ fontSize: 15, fontWeight: 700, color: s.hitung.nilai_bersih < 0 ? "var(--danger)" : "var(--text-primary)", fontVariantNumeric: "tabular-nums" }}>
                {formatRupiah(s.hitung.nilai_bersih)}
              </span>
            </div>
