...
// ============================================================================
// Kas & Pengeluaran — Portal Admin/Direktur (Task 17, Tahap 3). Salinan APA
// ADANYA dari `pm-portal/keuangan/kas/page.tsx` (Task 33 PM) — 2 tab (Akun
// Kas / Pengeluaran) lewat `<SegmentedTab>` + tombol Transfer/Pengeluaran +
// BottomSheet form. Bentuk respons SUDAH diverifikasi baris-per-baris ke
// `apps/api/src/routes/v1/cash.ts` saat Task 33 PM (lihat komentar kepala
// `_bersama/tipe.ts` di atas definisi `RespCashSummary` dst) — diverifikasi
// ULANG di sini untuk cabang yang beda dari PM (lihat bawah).
//
// ── BEDA dari sumber PM (Task 17, BUKAN sekadar gerbang — lihat `[id]/
//    page.tsx` untuk penambahan fungsional utamanya) ──────────────────────
//
// `daftarProyek` TIDAK memfilter `.filter((p) => p.pm)` — pola sama Task
// 7/9/10/11/14 (PM hanya melihat proyek miliknya sendiri; admin/direktur
// company-wide).
//
// admin+direktur SAMA-SAMA punya `cash:account:manage` (dikonfirmasi live
// query Task 13) yang PM TIDAK PUNYA — itu sebabnya halaman [id] di portal
// ini punya tombol "Batalkan" transfer yang sengaja TAK dibangun di PM
// Portal. Halaman INI (bukan [id]) tidak butuh gerbang baru: membuat
// transfer (`cash:transfer:create`) dan mencatat pengeluaran (`POST /cash/
// expenses`, hanya `authenticate`) sama-sama sudah dipegang PM juga.
//
// ⚠️ `autoApprove` pengeluaran (`cash.ts:565`) HANYA `role === 'admin' ||
// role === 'pm'` — PERSIS begitu, TIDAK menyebut `direktur` sama sekali.
// Diverifikasi ulang sendiri (Task 17 Step 1): akun `direktur` yang mencatat
// pengeluaran lewat form ini akan mendapat status "submitted" (menunggu
// approval), BUKAN "approved" otomatis seperti admin/pm — meski keduanya
// memegang `cash:account:manage` yang sama. Ini business rule (bukan
// authorization gate, ADR-004 Rule #6), jadi TIDAK digerbang di klien;
// server yang memutuskan status akhirnya dan UI menampilkan apa adanya lewat
// `StatusBadge`.
//
// TIDAK menambah tombol approve/reject pengeluaran di halaman ini — WARISI
// keputusan PM (Temuan #2 Task 31 PM): approve/reject `cash:expense:approve`
// HANYA lewat Inbox terpusat (Task 4 plan ini), BUKAN diduplikasi di modul
// Kas. `cash:expense:approve` admin+direktur SAMA-SAMA punya (tabel Task
// 13), tapi jalurnya tetap satu pintu.
//
// State galat AKSI (`galatForm`, dua BottomSheet) TERPISAH dari galat MUAT
// (`galatAkun`/`galatExpense` dari `useData`) — pelajaran Task 31 PM,
// diwarisi seluruh Tahap 3.
// ============================================================================
...
export default function AdminKasPage() {
  ...
  async function kirimExpense() {
    ...
    try {
      ...
      // WAJIB untuk multipart, bukan opsional (dikonfirmasi Task 31/33 PM).
        ...
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {(dataAkun?.accounts ?? []).map((a) => (
              <Link key={a.id} href={`/admin-portal/keuangan/kas/${a.id}`} style={{ textDecoration: "none" }}>
                <div style={{ background: "var(--surface)", borderRadius: 16, padding: "var(--pad-kartu-lega)", border: "1px solid var(--border)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)", fontVariantNumeric: "tabular-nums" }}>{formatRupiah(a.balance)}</span>
                  </div>
                </div>
              </Link>
            ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <select value={formTransfer.from_account_id} onChange={(e) => setFormTransfer((f) => ({ ...f, from_account_id: e.target.value }))}
              ...
              {(dataAkun?.accounts ?? []).map((a) => <option key={a.id} value={a.id}>{a.name} ({formatRupiah(a.balance)})</option>)}
            </select>
          </label>
          <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
