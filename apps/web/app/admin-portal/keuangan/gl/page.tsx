...
// Portal Admin/Direktur (Task 15, Tahap 3). Salinan APA ADANYA dari
// `pm-portal/keuangan/gl/page.tsx` (Task 34 PM) — 4 tab, BottomSheet "Akun
// Baru"/"Jurnal Manual", validasi `seimbangLini` sisi klien, batas 2+ baris —
// SEMUA logic form tak berubah.
//
// ── INI TASK PALING KRITIS Tahap 3 — GL benar-benar terbelah admin vs direktur
//
// Live query Task 13 (2026-08-22): direktur punya `gl:view`/`gl:periode:*`,
// TAPI TIDAK `gl:manage`/`gl:post`/`gl:void` sama sekali. PM Portal (sumber
// salinan di atas) TIDAK punya render-gate karena PM di modul ini punya akses
// PENUH — jadi gerbang di bawah ini DITAMBAHKAN di sini, bukan disalin dari
// yang sudah ada.
//
// `bolehTulisGl` menggerbang KEDUA tombol header ("+ Akun"/"+ Jurnal") —
// keduanya butuh `gl:manage` (`gl.ts:71` POST /accounts, `gl.ts:168` POST
// /journal-entries). TIDAK DIRENDER (bukan `disabled`) saat direktur tak
// punya izin, pola PERSIS `bolehApprove` Task 10 Change Order
// (`kontrak/change-order/page.tsx`) — JANGAN "perbaiki" jadi selalu tampil
// dengan asumsi "direktur biasanya subset admin". Tab "Bagan Akun"/"Buku
// Besar"/"Neraca & Laba-Rugi" dan LIST jurnal TETAP dirender tanpa gerbang
// tambahan — keempat GET di bawah ini cuma butuh `gl:view`, yang admin DAN
// direktur SAMA-SAMA punya:
//   GET /gl/accounts (gl.ts:57), GET /gl/journal-entries (gl.ts:120),
//   GET /gl/ledger (gl.ts:327), GET /gl/laporan (gl.ts:567)
// — hanya AKSI TULIS yang tersembunyi.
//
// Tombol Posting/Batalkan Jurnal (`gl:post`/`gl:void`) ada di halaman detail
// `gl/jurnal/[id]/page.tsx`, BUKAN di sini.
//
// Bentuk respons DIVERIFIKASI baris-per-baris ke `apps/api/src/routes/v1/
// gl.ts` + `apps/api/src/lib/laporan-keuangan.ts` saat Task 34 PM (bukan
// hanya membaca brief) — diwarisi apa adanya, tipe di `_bersama/tipe.ts`.
//
...
// tombol aksi utama di repo ini (`uji-tombol-primer-seragam.mjs`).
//
...
// `useData`) — pelajaran Task 31 PM, sama seperti halaman keuangan lain di
// portal ini.
//
// TANPA `useSearchParams` — tab dikendalikan `useState` lokal, jadi halaman
// ini tak butuh <Suspense>.
//
// Satu-satunya beda TEKSTUAL dari sumber PM di luar gerbang: dua
// `padding: 16` (kartu Neraca/Laba-Rugi) diganti `padding: "var(--pad-kartu-
// lega)"` — NILAI SAMA (16px), murni literal→token supaya tak menambah
// pelanggaran `kerapatan-ratchet.mjs` (constraint global task ini). Tak ada
// perubahan visual maupun logic.
// ============================================================================
...
export default function AdminGlPage() {
  ...
  // akun/jurnal baru. TIDAK DIRENDER (bukan disabled) saat tak ada, pola sama
  ...
  async function buatJurnal() {
    ...
    if (!seimbangLini) {
      setGalatForm(`Debit (${formatRupiah(totalDebitLini)}) harus sama dengan Kredit (${formatRupiah(totalKreditLini)}) sebelum di-posting — jurnal boleh tersimpan draf tak seimbang, tapi lebih aman disamakan dari awal.`);
      return;
    }
    setMengirim(true);
    setGalatForm(null);
      ...
      {tab === "jurnal" && (
        <>
          ...
          {!memuatJurnal && !galatJurnal && dataJurnal && dataJurnal.data.length === 0 && (
            <EmptyState icon={BookOpen} judul="Belum ada jurnal" deskripsi={bolehTulisGl ? "Buat jurnal manual pertama lewat tombol + Jurnal di atas." : "Belum ada jurnal tercatat di badan usaha ini."} />
          )}
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {(dataJurnal?.data ?? []).map((j) => (
              <Link key={j.id} href={`/admin-portal/keuangan/gl/jurnal/${j.id}`} style={{ textDecoration: "none" }}>
      ...
      {tab === "akun" && (
        <>
          ...
          {!memuatAkun && !galatAkun && dataAkun && dataAkun.data.length === 0 && (
            <EmptyState icon={BookOpen} judul="Belum ada akun" deskripsi={bolehTulisGl ? "Buat akun pertama lewat tombol + Akun di atas." : "Belum ada akun tercatat di badan usaha ini."} />
          )}
          ...
          {!memuatLedger && dataLedger && dataLedger.data.length > 0 && (
            <>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", fontSize: 12, color: "var(--text-secondary)" }}>
                <span>Debit: <strong style={{ color: "var(--text-primary)" }}>{formatRupiah(dataLedger.meta.total_debit)}</strong></span>
                <span>Kredit: <strong style={{ color: "var(--text-primary)" }}>{formatRupiah(dataLedger.meta.total_credit)}</strong></span>
                <span style={{ color: Math.abs(dataLedger.meta.selisih) < 0.01 ? "var(--success)" : "var(--danger)", fontWeight: 700 }}>
                  Selisih: {formatRupiah(dataLedger.meta.selisih)}
                </span>
              </div>
              {/* Endpoint `/gl/ledger` TIDAK punya flag "terpotong" (beda dari
                  ...
                  (task-15-report.md), bukan ditambal di sini. */}
                  ...
                  <tbody>
                    {dataLedger.data.map((b, i) => (
                      <tr key={`${b.entry_id}-${i}`} style={{ borderBottom: "1px solid var(--border)" }}>
                        <td style={{ padding: "var(--pad-baris)" }}>{formatTanggal(b.entry_date)}</td>
                        ...
                        <td style={{ padding: "var(--pad-baris)", textAlign: "right" }}>
                          {b.debit > 0 ? formatRupiah(b.debit) : "—"}
                        </td>
                        <td style={{ padding: "var(--pad-baris)", textAlign: "right" }}>
                          {b.credit > 0 ? formatRupiah(b.credit) : "—"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <>
              {!dataLaporan.neraca.seimbang && (
                <div role="alert" style={{
                  ...
                  Neraca TIDAK seimbang — selisih {formatRupiah(dataLaporan.neraca.selisih)}. Ada
                  ...
                  <div key={key} style={{ marginBottom: 10 }}>
                    ...
                    {kel.akun.map((a) => (
                      <div key={a.account_id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "4px 0" }}>
                        ...
                        <span style={{ fontVariantNumeric: "tabular-nums" }}>{formatRupiah(a.saldo)}</span>
                      </div>
                    ))}
                    <div style={{
                      ...
                      <span style={{ fontVariantNumeric: "tabular-nums" }}>{formatRupiah(kel.total)}</span>
                    </div>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: 700, color: "var(--navy)" }}>
                  <span>Laba Berjalan</span>
                  <span style={{ fontVariantNumeric: "tabular-nums" }}>{formatRupiah(dataLaporan.neraca.labaBerjalan)}</span>
                </div>
              </div>
              ...
              <div style={{ background: "var(--surface)", borderRadius: 16, padding: "var(--pad-kartu-lega)", border: "1px solid var(--border)" }}>
                ...
                <div style={{
                  ...
                  <span style={{ fontVariantNumeric: "tabular-nums" }}>{formatRupiah(dataLaporan.labaRugi.labaBersih)}</span>
                </div>
              </div>
            </>
          )}
          ...
          <BottomSheet terbuka={sheetJurnal} onTutup={() => setSheetJurnal(false)} judul="Jurnal Manual">
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: seimbangLini ? "var(--success)" : "var(--text-secondary)" }}>
                <span>Debit: {formatRupiah(totalDebitLini)}</span>
                <span>Kredit: {formatRupiah(totalKreditLini)}</span>
              </div>
