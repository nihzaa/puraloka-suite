...
// ============================================================================
// Jadwal & Baseline — Portal Admin/Direktur (Task 11, Tahap 2). PER-PROYEK,
// disalin dari `pm-portal/jadwal/page.tsx` — dua bagian yang endpoint-nya
// BEDA modul, digabung satu halaman:
//
...
//    lihat alasannya di komentar berkas itu). Admin/direktur pakai permission
//    yang sama (`projects:view`), jadi dipakai apa adanya, baca saja di
//    halaman ini.
//
// 2. Baseline — `apps/api/src/routes/v1/baseline-jadwal.ts`. Tulis lewat
//    `projects:baseline:manage` — admin DAN direktur SAMA-SAMA punya izin ini
//    (live 2026-08-22), jadi keduanya BISA menetapkan baseline baru
...
//
// 10 interface CPM/histogram/method-statement/baseline di bawah tetap LOKAL
// di berkas ini (BUKAN dipindah ke `_bersama/tipe.ts`) — mengikuti sumber
// aslinya (`pm-portal/jadwal/page.tsx`) apa adanya, dan genuinely spesifik
// untuk satu halaman ini saja (tak dipakai halaman admin lain lainnya),
          ...
          {!memuatCpm && pekerjaan.map((p) => (
            <div key={p.id} style={{ padding: 14, borderRadius: 14, background: "var(--surface)", border: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>
                {formatTanggal(p.mulaiPalingAwal)} — {formatTanggal(p.selesaiPalingAwal)} · {p.durasi} hari
        ...
        <>
          ...
          {!memuatPergeseran && dataPergeseran?.baseline && (
            <div style={{ padding: "var(--pad-kartu-lega)", borderRadius: 16, background: "var(--surface)", border: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>
                Ditetapkan {formatTanggal(dataPergeseran.baseline.ditetapkan_pada)}
              </div>
