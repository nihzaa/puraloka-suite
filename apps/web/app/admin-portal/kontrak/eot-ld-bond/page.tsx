...
// ============================================================================
// EOT + Denda Keterlambatan + Register Jaminan — Portal Admin/Direktur
// (Task 9, Tahap 2). Salinan APA ADANYA dari
// `pm-portal/kontrak-lengkap/eot-ld-bond/page.tsx` (Task 13) — endpoint
...
export default function AdminEotLdBondPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--gap-bagian)" }}>
      <KepalaPortal judul="EOT, Denda &amp; Jaminan" />
        ...
        <>
          ...
          {dataEot?.meta && (
            <div style={{ fontSize: 12, color: "var(--text-secondary)", padding: 10, borderRadius: 10, background: "var(--surface-subtle)" }}>
              Tanggal efektif: {formatTanggal(dataEot.meta.tanggalEfektif)} (asli {formatTanggal(dataEot.meta.tanggalAsli)} + {dataEot.meta.totalHariEOT} hari EOT disetujui)
          ...
          {!memuatEot && dataEot?.data.map((e) => (
            <div key={e.id} style={{ padding: "var(--pad-kartu-lega)", borderRadius: 16, background: "var(--surface)", border: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {!dataLd.data.adaDenda ? (
                ...
                <div style={{ padding: "var(--pad-kartu-lega)", borderRadius: 16, background: "var(--surface)", border: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: 10 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 22, fontWeight: 700, color: "var(--danger)" }}>{formatRupiah(dataLd.data.denda)}</div>
                  </div>
                  <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>
                    Telat {dataLd.data.hariTelat} hari · dasar {formatRupiah(dataLd.data.dasarPerhitungan)} · batas {formatRupiah(dataLd.data.batasNominal)}
                  </div>
                </div>
              )}
            </div>
      ...
      {proyekAktif && tab === "bond" && (
        <>
          ...
          {!memuatBond && dataBond?.data.map((b) => (
            <div key={b.id} style={{ padding: "var(--pad-kartu-lega)", borderRadius: 16, background: "var(--surface)", border: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                <div>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: "var(--navy)" }}>{formatRupiah(b.amount)}</div>
              <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>
                {formatTanggal(b.issued_date)} — {formatTanggal(b.expiry_date)}
              </div>
              {b.status === "aktif" && (
                <div style={{ display: "flex", gap: 8 }}>
                  <button
      ...
      <BottomSheet terbuka={sheetBond} onTutup={() => setSheetBond(false)} judul="Jaminan Baru">
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>
            Jenis
            <Pilihan value={formBond.bond_type} onChange={(e) => setFormBond((f) => ({ ...f, bond_type: e.target.value }))}
              style={{ width: "100%", marginTop: 6, minHeight: 44, padding: "0 12px", borderRadius: 12, border: "1px solid var(--border)", fontSize: 14, background: "var(--surface)", color: "var(--text-primary)", boxSizing: "border-box" }}>
