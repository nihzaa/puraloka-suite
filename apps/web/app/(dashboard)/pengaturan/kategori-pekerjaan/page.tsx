...
const inputStyle: React.CSSProperties = {
  ...
  fontSize: 13, outline: "none", background: "var(--surface)", color: C.text, boxSizing: "border-box", fontFamily: "inherit",
};
...
function KategoriContent() {
  const canManage = hasPerm("work_categories:manage");
  ...
  const load = useCallback(async () => {
    try {
      const { data } = await api.get<{ categories: WorkCategoryRow[] }>("/api/v1/work-categories", { params: { all: true } });
      setRows(data.categories ?? []);
    } catch { setToast({ type: "error", msg: "Gagal memuat kategori" }); }
    ...
    <div style={{ ...card, marginBottom: 18, padding: 18 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: 12, alignItems: "end" }}>
        <div>
          ...
          <input id="code" value={code} onChange={e => setCode(e.target.value)} placeholder="cth: mep" style={inputStyle} />
        ...
        <div>
          ...
          <input id="label" value={label} onChange={e => setLabel(e.target.value)} placeholder="cth: Mekanikal Elektrikal Plumbing" style={inputStyle} />
        </div>
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        <button onClick={submit} disabled={saving || dup} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, border: "none", background: saving || dup ? "var(--text-muted)" : C.navy, color: "#fff", fontSize: 13, fontWeight: 600, cursor: saving || dup ? "not-allowed" : "pointer" }}>
...
function RowItem({ row, canManage, onSaved, onError }: { row: WorkCategoryRow; canManage: boolean; onSaved: () => void; onError: (m: string) => void }) {
  ...
  const rowStyle: React.CSSProperties = {
    display: "grid", gridTemplateColumns: "160px 1fr 70px 90px", gap: 12, padding: "10px 20px",
