...
function RowItem({ row, canManage, onSaved, onError }: { row: KasbonPurposeRow; canManage: boolean; onSaved: () => void; onError: (m: string) => void }) {
  ...
  const rowStyle: React.CSSProperties = { display: "grid", gridTemplateColumns: "160px 1fr 70px 90px", gap: 12, padding: "8px 20px", borderBottom: `1px solid ${C.border}`, alignItems: "center", fontSize: 13, opacity: row.is_active ? 1 : 0.55 };
