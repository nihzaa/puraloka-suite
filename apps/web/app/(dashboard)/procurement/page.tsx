      ...
      {rejectId && (
        <Modal title="Tolak Material Request" onClose={() => setRejectId(null)} width={440}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div>
              <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: C.mid, marginBottom: 4 }}>Alasan Penolakan (opsional)</label>
              <textarea value={rejectNotes} onChange={e => setRejectNotes(e.target.value)} rows={3} style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: `1px solid ${C.border}`, fontSize: 14, resize: "vertical", boxSizing: "border-box" }} placeholder="cth: Kuantitas berlebihan, perlu revisi..." />
            </div>
            ...
            {items.map((item, idx) => (
              <div key={idx} style={{ background: C.bg, borderRadius: 10, padding: 12, border: `1px solid ${C.border}` }}>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr auto", gap: 8, alignItems: "end" }}>
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 500, color: C.mid, marginBottom: 4 }}>Qty *</label>
                    <input type="number" value={item.qty} onChange={e => updateItem(idx, "qty", e.target.value)} style={{ width: "100%", padding: "9px 10px", borderRadius: 8, border: `1px solid ${C.border}`, fontSize: 13, boxSizing: "border-box" }} placeholder="0" />
                  </div>
                  <div>
            ...
            {items.map((item, idx) => (
              <div key={idx} style={{ background: C.bg, borderRadius: 10, padding: "10px 12px", border: `1px solid ${C.border}` }}>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr auto", gap: 8, alignItems: "end" }}>
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 500, color: C.mid, marginBottom: 4 }}>Qty *</label>
                    <input type="number" value={item.qty} onChange={e => updateItem(idx, "qty", e.target.value)} style={{ width: "100%", padding: "9px 10px", borderRadius: 8, border: `1px solid ${C.border}`, fontSize: 13, boxSizing: "border-box" }} />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 500, color: C.mid, marginBottom: 4 }}>Satuan *</label>
                    <select aria-label="Satuan" value={item.unit} onChange={e => updateItem(idx, "unit", e.target.value)} style={{ width: "100%", padding: "9px 10px", borderRadius: 8, border: `1px solid ${C.border}`, fontSize: 13, background: C.surface }}>
