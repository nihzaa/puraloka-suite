...
export const PETA_MENU: GrupMenu[] = [
  {
    ...
    items: [
      ...
      { key: 'md-coa', label: 'Chart of Accounts', status: 'hidup', href: '/akuntansi', guna: 'Bagan akun untuk buku besar.', catatan: 'Migrasi 167 (tenant-aware) + seed 170. Migrasi 047 DIPENSIUNKAN lewat R-001 — catatan lama "sengaja belum di-apply" SALAH dan berbahaya: yang memercayainya akan bekerja melawan ratifikasi.' },
      ...
      { key: 'md-kalender', label: 'Kalender Kerja', status: 'hidup', href: '/jadwal', guna: 'Hari libur & hari kerja efektif — dasar hitung durasi dan denda.', catatan: 'Pola mingguan per-company/proyek + hari libur. Libur ber-`tetap_bekerja` TETAP hari kerja: jejaknya disimpan (menentukan tarif upah) tanpa menggeser jadwal.' },
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'kt-rfi', label: 'Request for Information', status: 'hidup', href: '/kontrak/rfi', guna: 'Pertanyaan resmi ke konsultan/pemberi kerja beserta jawabannya.', catatan: 'Capability Tier-2 (ROADMAP #24). Berbeda dari RFI lapangan: yang dihitung di sini adalah berapa lama pekerjaan menggantung menunggu jawaban — angka yang dibawa ke klaim EOT.' },
      ...
      { key: 'kt-asuransi', label: 'Register Asuransi', status: 'hidup', href: '/kontrak/asuransi', guna: 'Polis CAR/TPL beserta masa berlakunya.', catatan: 'Migrasi 199 (`polis_asuransi`) · `lib/register-asuransi.ts` · /kontrak/asuransi. Celah tanggal antar-polis dihitung, bukan sekadar didaftar.' },
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'jd-cpm', label: 'Jalur Kritis (CPM)', status: 'hidup', href: '/jadwal', guna: 'Rantai pekerjaan yang menentukan tanggal selesai.', catatan: 'Empat jenis relasi (FS/SS/FF/SF) + jeda. Lingkaran dependensi DINYATAKAN dan jalur kritisnya dikosongkan, bukan dikarang. Float negatif sebanding dengan besar keterlambatan — bukan -1 untuk semua.' },
      ...
      { key: 'jd-histogram', label: 'Histogram Sumber Daya', status: 'hidup', href: '/jadwal', guna: 'Kebutuhan tenaga & alat per periode.', catatan: 'Yang dilaporkan PUNCAK, bukan rata-rata: 40 orang di minggu 7 dan 4 di minggu 8 punya rata-rata 22 — angka yang tak pernah terjadi dan menyembunyikan kekurangan 15 orang.' },
      ...
      { key: 'jd-method', label: 'Method Statement', status: 'hidup', href: '/jadwal', guna: 'Cara kerja yang disetujui untuk pekerjaan berisiko.', catatan: 'Penolakan WAJIB beralasan (>=10 huruf) dan keputusan wajib bertanggal — constraint DB, bukan aturan UI. Kolom pengendalian risiko K3 ditandai merah kalau kosong.' },
    ],
  },
  {
    ...
    items: [
      ...
      { key: 'cc-contingency', label: 'Manajemen Contingency', status: 'hidup', href: '/keuangan/contingency', guna: 'Cadangan risiko: berapa tersisa, dipakai untuk apa.', catatan: 'Migrasi 200 · `lib/contingency.ts` · /keuangan/contingency. Defisit ditampilkan negatif, bukan diratakan ke nol.' },
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'sk-tender', label: 'Tender Subkontraktor', status: 'hidup', href: '/mandor/tender', guna: 'Memilih subkon lewat penawaran, bukan penunjukan.', catatan: 'Migrasi 201+203 · `lib/tender-subkon.ts` · /mandor/tender. Menjawab kenapa borongan jatuh ke penawar tertentu.' },
      ...
      { key: 'sk-retensi', label: 'Retensi Subkon', status: 'hidup', href: '/mandor/retensi', guna: 'Tahanan pembayaran subkon sampai masa pemeliharaan lewat.', catatan: 'Migrasi 183+188 · `lib/retensi-subkontrak.ts` · /mandor/retensi. Catatan lama "ke subkon belum" SALAH sejak migrasi 183.' },
      ...
      { key: 'sk-evaluasi', label: 'Evaluasi Subkon', status: 'hidup', href: '/kepatuhan', guna: 'Menilai mutu & ketepatan waktu subkon.', catatan: 'Lima dimensi berbobot (K3 & kepatuhan 25%+20%). Kecelakaan kerja MENGGUGURKAN, bukan diratakan skor: subkon berskor K3 80 dengan satu kecelakaan bukan subkon yang aman.' },
      { key: 'sk-kepatuhan', label: 'Kepatuhan Subkon', status: 'hidup', href: '/kepatuhan', guna: 'Memastikan izin, asuransi, dan pajak subkon berlaku.', catatan: 'Dokumen bercentang `terverifikasi` yang masa berlakunya HABIS ditandai khusus — centang itu hanya berarti seseorang pernah memeriksanya, bukan bahwa dokumennya masih hidup hari ini.' },
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'lp-permit', label: 'Izin Kerja', status: 'hidup', href: '/kepatuhan', guna: 'Work permit untuk pekerjaan berisiko tinggi.', catatan: 'Pemutus WAJIB berbeda dari pengaju (constraint DB + permission terpisah `k3:permit:decide`). Izin berstatus `disetujui` yang jendela waktunya lewat ditandai TIDAK BERIZIN — izin kerja bukan dokumen abadi.' },
      { key: 'lp-rfi', label: 'Request for Inspection', status: 'hidup', href: '/lapangan/inspeksi', guna: 'Permintaan pemeriksaan sebelum pekerjaan ditutup.', catatan: 'Capability Tier-2 (ROADMAP #24). Yang memutuskan lolos terpisah dari yang mengajukan; gagal boleh langsung jadi temuan punch list.' },
      { key: 'lp-submittal', label: 'Submittal Register', status: 'hidup', href: '/lapangan/submittal', guna: 'Pengajuan contoh material & gambar kerja untuk disetujui.', catatan: 'Capability Tier-2 (ROADMAP #24). Revisi dirantai ke pengajuan pertama — "ditolak 3× sebelum disetujui" adalah fakta yang menjelaskan keterlambatan pengadaan. Persetujuan lewat Workflow Engine, bukan status sendiri.' },
  ...
  },
  {
    ...
    items: [
      { key: 'fn-gl', label: 'Buku Besar', status: 'hidup', href: '/akuntansi', guna: 'Jurnal seluruh transaksi — sumber laporan keuangan resmi.', catatan: 'Migrasi 167–175 · `routes/v1/gl.ts` (bagan akun, jurnal, posting, void, buku besar, neraca saldo, laporan) · halaman /akuntansi.' },
      { key: 'fn-jurnal', label: 'Jurnal Umum', status: 'hidup', href: '/akuntansi', guna: 'Input jurnal manual untuk penyesuaian.', catatan: 'Input jurnal manual + posting + void hidup di /akuntansi. Debit-kredit dijaga constraint DB, bukan hanya validasi form.' },
