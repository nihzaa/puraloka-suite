...
- **OPEN-4 🔴→✅ Fitur foto TIDAK PERNAH BERFUNGSI — DIPERBAIKI (PR #40).** Bucket `project-photos`/`kasbon-photos` direferensikan web tapi **tidak pernah ada** di `storage.buckets`. **Bukti fitur tak pernah jalan:** `project_photos` 36 baris tapi **semua URL `images.unsplash.com` (seed dummy)**, `kasbons.photo_url` terisi **0**, nol objek storage. Lebih buruk: di **mandor-portal** (jalur lapangan) kegagalan **SENYAP** — `catch {}` kosong, foto di-drop, progress log tetap tersimpan tanpa foto. **Fix (migration 098):** 2 bucket dibuat PRIVAT + policy `service_role`-only sejak awal (pola 097, tidak mengulang `bucket_id`-only yang bocor); upload dialihkan lewat API (`POST /projects/:id/photos/upload`, `/mandor/kasbon-photo/upload` — base64 + validateMime + cap 10MB + signed URL); nol akses storage langsung tersisa di web; kegagalan tak lagi senyap (submit dibatalkan + pesan). **Verifikasi live:** service_role bisa upload + signed URL OK, anon & authenticated ditolak. **CLAUDE.md dikoreksi** (fitur ditandai SELESAI padahal tak pernah jalan).

- **OPEN-5 ✅ Jaring otorisasi wiring — DITUTUP (PR #40).** Sebelumnya nol test HTTP yang assert 403 per endpoint (hanya logika `has_permission` ter-test), padahal preHandler = satu-satunya penegak live. **Ditambah 22 test integrasi** (`authz-endpoints.test.ts`) untuk 11 endpoint sensitif × positif+negatif, mendaftarkan route module ASLI (wiring nyata) + `users`/`get_role_permissions` ASLI; hanya verifikasi token yang di-stub. **Mutation-tested:** menghapus `requirePermission` dari CO approve → test NEGATIF merah (jaring terbukti nyata).
