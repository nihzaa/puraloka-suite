...
- **Tabel workflow_* yatim (pasca CONTRACT 1C, PR #34):** setelah dual-write shadow diretire, `workflow_definitions/states/transitions/instances` + `approval_delegations` nol pembaca/penulis bisnis. Lihat AUDIT_REPORT OPEN-2 + ADR-006. Rekomendasi: drop lewat migration terpisah **setelah keputusan founder** (migration 092 sempat drop → dikembalikan 093 karena founder menahan keputusan).
