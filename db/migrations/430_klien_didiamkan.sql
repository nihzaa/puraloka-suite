...
BEGIN
  ...
  SELECT count(*) INTO n FROM notification_rules
   WHERE event_type = 'klien_didiamkan' AND is_active;
  IF n <> n_aktif THEN
  ...
  SELECT count(*) INTO n FROM company_settings WHERE key = 'otomasi.klien_didiamkan.hari';
  IF n <> n_aktif THEN
  ...
  */
  ...
  SELECT count(*) INTO n FROM jadwal_tugas
   WHERE tugas = 'klien-didiamkan' AND aktif AND jenis = 'mingguan';
  IF n <> n_ang THEN
