...
| `audit-daftar-berhalaman-jujur.mjs` | daftar yang BERHALAMAN wajib mengaku berhalaman — rute memakai `.range(off, off+lim-1)` lalu membalas hanya barisnya, sehingga klien TAK PUNYA CARA tahu masih ada sisa. Daftar yang berhenti di baris ke-30 tak bisa dibedakan dari daftar yang memang cuma punya 30: tak ada galat, tak ada tanda, dan pembacanya mengambil keputusan dari sebagian data tanpa tahu ia sebagian. Diukur 2026-09-13: `/api/v1/notifications` **10.767 baris**, yang terlihat dari HP tiga puluh — kelas cacat yang sudah dibayar sekali (§8a.3: "8.947 baris di basis, 30 yang bisa dilihat, nol tanda"). ⚠ BUKAN duplikat `audit-baca-tak-terpotong.mjs` di atasnya: yang itu menjaga pembacaan PENUH tak terpotong senyap di 1.000 baris PostgREST; yang ini kebalikannya — pembacaan yang memang SENGAJA berhalaman tetapi tak mengaku. Cacatnya hidup di CELAH antara kedua penjaga. ⚠ Dua pengukuran pertama saya keduanya SALAH dan ke arah berlawanan: per-BERKAS memberi 7 (satu rute ekspor menyebut `total` → seluruh berkas dianggap jujur), per-handler tanpa saringan memberi 43 (menyapu 26 rute otomasi yang bukan daftar manusia), dan versi pertama pendeteksinya menuduh 7 rute yang SUDAH jujur karena hanya mencari `meta:` bukan `total:` telanjang. Yang benar 8, kini 4. Kontraknya MENGIKUTI `audit.ts` (`meta: { total, page, limit, pages }`, `count` dari query yang SAMA), bukan bentuk baru — dua bentuk paginasi di satu API berarti tiap klien harus tahu rute mana memakai yang mana. ⚠ Batas: yang dibaca BENTUK BALASAN di kode, bukan balasan sungguhan — ia tak tahu apakah `total`-nya BENAR, hanya bahwa ia dijanjikan (ratchet; lantai menyimpan DAFTAR NAMA) |
...
| `audit-modul-mobile-nyata.mjs` | tiap entri "Lainnya" wajib menunjuk LAYAR NATIVE yang ada, atau jadi baris MATI — tak ada keadaan ketiga. **Premisnya DIGANTI 2026-09-11** bersama pencabutan WebView: versi lama menjaga tiap modul WebView menunjuk halaman web ber-`page.tsx`, dan ia hijau sampai akhir sebab ketujuh belas halamannya MEMANG ada — yang tak bisa ia lihat: apakah halamannya TAMPIL. Bentuk cacatnya tak berubah oleh pergantian teknologi: **peta di satu berkas, tujuannya di berkas lain, tak ada yang menghubungkan**. Yang diperiksa kini: `nativeJalur` punya berkas layarnya (⚠ `tsc` menangkap ini juga lewat tipe `Href`, TAPI hanya bila `.expo/types/router.d.ts` sudah diregenerasi — berkas itu pernah basi enam hari, jadi hijaunya bisa berarti "belum tahu"), entri tanpa `nativeJalur` tak punya jalur navigasi apa pun, dan nol `router.push` yang menebak jalur dari kunci. Yang kedua tak terlihat `tsc` sama sekali, dan justru inti keputusan founder: pintu yang bisa ditekan lalu gagal adalah yang menghapus kepercayaan. Diukur sesudah gelombang 1: 9 native · 11 baris mati · 0 buntu (ambang NOL) |
...
| `audit-keparahan-sepakat.mjs` | daftar "NCR berat" wajib SEPAKAT di api + web, dan wajib meliputi seluruh enum `ncr_severity` — tiga tempat menghitungnya dengan pola teks `/major|mayor|tinggi|high/i`, yang melewatkan `kritis`: tingkat PALING parah. Diukur 2026-09-13 atas NCR TERBUKA: 6 kritis + 5 major, dan ringkasan melaporkan **5 dari 11**. Enam temuan paling mendesak tak terhitung, dan angka yang terlalu KECIL terbaca persis seperti kabar baik. Nol gejala: `kritis` nilai enum yang SAH — query berhasil, tsc hijau, yang salah cuma angkanya. Pola itu menjaga `high` dan `tinggi` yang tak pernah ada di basis ini sambil melewatkan yang ada di KEDUA enum — ia menjaga kosakata yang DIBAYANGKAN, bukan yang dipakai. ⚠ Mengukur dampaknya menemukan cacat KEDUA yang lebih besar: penyaring "terbuka" membandingkan dengan `closed`/`selesai`, dan KEDUANYA tak ada di enum — punch karenanya melaporkan **40 terbuka dari 40 total**, dua angka yang selalu sama dan tak seorang pun melihatnya sebab masing-masing masuk akal sendiri. Kesepakatan ditegakkan penjaga, bukan impor bersama: `packages/shared` kosong dan web tak pernah mengimpor dari apps/api (ambang NOL) |
...
| `audit-mobile-tanpa-webview.mjs` | apps/mobile tak boleh memakai WebView lagi — keputusan founder 2026-09-11 (*"suka gagal dan ga nampil"*). Diukur ke produksi hari itu: **SELURUH 17 modul** menjawab `307 → /login`, bukan sebagian. Yang membuatnya tak layak dipertahankan bukan satu bug melainkan BENTUKNYA — tiga lapis yang masing-masing BENAR sendiri (token ditanam klien, gerbang cookie di `middleware.ts`, SSR Next.js yang berjalan sebelum satu baris JS halaman ada), dan yang patah cuma sambungannya, tanpa galat di lapisan mana pun. Menggantikan `audit-sesi-webview-nyambung.mjs` yang dihapus bersama subjeknya: penjaga itu hijau sampai akhir dan tetap BENAR — ia menjaga sambungan NAMA cookie, dan batas itu tertulis di kepalanya sendiri (*"bukan bukti sesi hidup"*). Dua penjaga yang keduanya jujur, dan cacatnya hidup di antara mereka. Memeriksa EMPAT jalur kembalinya: impor di kode, `dependencies` package.json, direktori rute `app/(app)/web/**`, dan `Tabs.Screen name="web/…"` yang tertinggal di layout. ⚠ Yang dicari `import`/`require`, BUKAN teks "webview" telanjang — komentar yang menerangkan kenapa ia dihindari tetap terhitung pemakaian kalau dipindai mentah (§8a.2). ⚠ Batas: yang dibaca KODE, bukan APK — dependensi transitif tak terlihat dari sini (ambang NOL) |
...
| `audit-haptik-terpakai.mjs` | haptik yang DIPASANG wajib DIPAKAI, dan di layar TULIS — `expo-haptics` terpasang 2026-09-12 lalu dipakai di SATU berkas (`BilahTab`). Bentuk cacat yang sama dengan font di atasnya: biaya penuh (paket ikut bundle), nol hasil, dan semua alat hijau sebab tak ada yang SALAH — cuma tak ada yang memanggilnya. Yang dijaga TEMPATNYA, bukan jumlahnya: menjaga "dipakai minimal sekali" akan hijau selamanya berkat satu pemanggilan di bilah tab. Ini aplikasi lapangan — HP di bawah matahari, sering bersarung tangan, dan konfirmasi VISUAL paling mudah terlewat di sana; tekanan yang tak terasa terjadi akan DIULANG (`audit-tekan-berumpan.mjs`: "dua NCR dari satu temuan"). ⚠ Layar tulis dibaca dari BENTUK kode (`api.post/put/patch` ATAU `antrekan(`), bukan daftar nama — dan itu bukan kehalusan: jalan pertamanya menemukan DUA layar yang penulisnya sendiri lewatkan (`izin-kerja/ajukan`, `ncr/lapor`) sebab keduanya memakai `antrekan(`. ⚠ Batas: yang dibaca KEPUTUSAN DI KODE — apakah getarannya terasa di perangkat tak terukur dari sini, dan bahkan MEMOTRET tak bisa menemukannya (haptik hidup ~100ms di jari). Ratchet; lantainya menyimpan DAFTAR NAMA, bukan cuma angka |
| `audit-kartu-pahlawan-tunggal.mjs` | SATU kartu pahlawan per layar, dan angka `displayBesar` (48px) hanya lahir di dalam komponennya — `ARAH-VISUAL-2026` §3d menulis aturannya sendiri sebagai yang **paling mudah dilanggar**: *"kalau tiga hal berwarna, tak ada yang menonjol, dan halaman kembali monoton dengan warna yang berbeda"*. Aturan yang cuma tertulis di dokumen akan dilanggar oleh orang yang tak pernah membaca dokumen itu. ⚠ Ditulis SEBELUM layar-layarnya dipindahkan, bukan sesudah: penjaga yang lahir sesudah pekerjaannya selesai hanya MENGESAHKAN apa pun yang sempat ditulis — yang ini sempat menolak pekerjaan penulisnya sendiri. Angka 48px yang digambar layar sendiri melewati `numberOfLines`, `adjustsFontSizeToFit`, pemilihan warna nada, dan pengukuran kontras di permukaan gelap — semuanya sudah dibayar mahal ("Rp 1.200.00" lalu "0", JOURNAL 2026-09-12). `HURUF.display` (38) sengaja TIDAK dilarang: ia tingkat di bawahnya dan sah dipakai di BARIS DAFTAR, dan melarangnya akan memerahkan layar yang sudah benar. ⚠ Batas: yang dibaca KEPUTUSAN DI KODE — apakah kartunya benar-benar menonjol hanya ketahuan dari MEMOTRET, dan potret itulah yang menemukan panah NAIK di sebelah kerugian Rp 97 juta pada jalan pertamanya (ambang NOL) |
...
| `audit-zona-waktu-terpasang.mjs` | container API wajib ber-`TZ=Asia/Jakarta` DAN paket `tzdata` — `node:22-alpine` berjalan di UTC, dan antara 00:00–07:00 WIB tanggal UTC masih HARI KEMARIN. Yang tergeser bukan tampilan melainkan TANGGAL TRANSAKSI: `new Date().toISOString().split('T')[0]` dipakai 140 tempat di `apps/api/src` melawan 14 pemakaian `todayWIB()`, dan DUA di antaranya tak punya cadangan dari klien sama sekali (`mandor.ts` `paid_at` pembayaran termin mandor, `settled_at` penyelesaian borongan) — di sana UTC satu-satunya sumber, tak bisa diselamatkan form. Laporan arus kas mengelompokkan per BULAN dari `paid_at`, jadi pembayaran pukul 01:00 WIB tanggal 1 masuk ember bulan SEBELUMNYA: totalnya tetap benar, batang bulanannya salah, nol galat. Terbukti sudah terjadi — tiga kasbon dibuat pukul 01:25 WIB tercatat bertanggal sehari sebelumnya. ⚠ `tzdata` ikut dijaga dan itu yang paling mudah terlewat: Alpine tak memuat basis zona waktu, jadi `TZ=Asia/Jakarta` TANPA paketnya jatuh DIAM-DIAM kembali ke UTC — setelan yang terlihat terpasang, lolos tiap pembacaan teks, dan tak berpengaruh apa pun (kelas yang sama dengan `audit-hook-eas-utuh.mjs`). ⚠ Batas: yang dibaca BERKAS KONFIGURASI, bukan container hidup — hijaunya BUKAN bukti produksi sudah WIB; itu dijawab `/health` atau `docker exec … date` (ambang NOL) |
...
| `audit-skema-pajak-lengkap.mjs` | tiap nilai enum `tax_scheme` wajib tertangani di tarif, label UI, dan tipe TS. Pola `scheme === 'ppn' ? A : B` memperlakukan nilai BARU sebagai cabang else — diukur 2026-09-04 saat `tanpa_pajak` ditambahkan: KEDUA `getTaxRate` berbunyi begitu, jadi proyek yang pajaknya sengaja dimatikan tetap dipotong PPh Final 2%. Angkanya sah, jurnalnya seimbang, yang salah cuma jumlah uang yang ditagihkan ke klien (ambang NOL) |
| `audit-dialog-bukan-bawaan.mjs` | `confirm()`/`alert()`/`prompt()` bawaan peramban dilarang di web — tombolnya berbahasa SISTEM OPERASI, tak bisa membedakan tindakan merusak dari yang biasa, MEMBEKUKAN seluruh tab, dan peramban MENEKAN dialog yang berulang sehingga `confirm()` memulangkan false diam-diam: tindakan yang seharusnya berjalan tak pernah berjalan, tanpa gejala. Pakai `tanya()`/`minta()`/`kabari()` dari `components/tanya` (ambang NOL) |
| `audit-dropdown-bisa-dicari.mjs` | dropdown bawaan dilarang di web — pakai `Pilihan` dari `components/pilihan`. Sebagian dropdown memuat RIBUAN pilihan (pemilih analisa Komposer: 3.040), dan elemen bawaan hanya bisa diloncati dengan mengetik huruf awal — orang yang tahu barangnya tapi tak hafal urutan katalog praktis tak bisa memakainya. 236 tag diganti 2026-09-04; isinya sama persis (option biasa), `onChange` tetap `e.target.value` (ambang NOL) |
