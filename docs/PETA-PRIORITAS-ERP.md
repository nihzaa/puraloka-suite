...
| `CECEP/` (48 file) | **AKTIF** | Planning CECEP SELESAI (Derived & Frozen); eksekusi M1–M4 struktur selesai; build order 10 langkah TERKUNCI di `MATERIAL-RAP-COMPANY-UI-DESIGN.md`; gerbang aktif: CI isolation → seed (butuh ack founder) |
...
|---|---|---|---|---|---|
| 1 | **Tuntaskan CECEP sampai siap-pakai** (build order 10 langkah: CI isolation → config → seed AHSP → endpoint hitung → take-off material → **RAP/pagu**) | Estimasi tetap Excel: tender lambat & rawan salah; tanpa pagu, belanja material tak terkendali — titik bocor #1 | Gate CI isolation (sedang berjalan) + ack seed founder | Besar | Phase 3 / Program C — **AKTIF** |
| 2 | **Tutup celah 3-way match** — fondasi sudah sebagian ada (guard over-receipt GR vs PO + invoice auto dari GR ter-link `goods_receipt_id`). Sisa: wajibkan link PO/GR di invoice manual, cek harga vs PO, dan cegah invoice dobel (auto-invoice `procurement.ts:659` tidak cek duplikat vs manual — komentar ≠ kode) | Invoice manual tanpa validasi = supplier bisa menagih melebihi/dobel, tak terdeteksi | Procurement (ada) | Kecil | Perpanjangan Modul 4; tidak bentrok CECEP (boleh diselipkan bila ada jeda menunggu gate) |
