...
| `CECEP/` (49 file) | **AKTIF** | Planning SELESAI (Derived & Frozen); build order 10 langkah TERKUNCI di `MATERIAL-RAP-COMPANY-UI-DESIGN.md`. Progres verified 2026-07-26/27 (PR #86–94): **langkah 1 CI isolation ✅ · 4 seed AHSP nasional ✅ (2.620 assemblies, verified 100%) · 5 endpoint hitung + golden ✅ · 2 config Lapis1/2 sebagian (PPN reuse; BUK/rounding BELUM di-config, masih wajib eksplisit per-request) · 8 AHSP Company sebagian (struktur DB ada, endpoint create-assembly sedang dikerjakan) · 10 UI sebagian (`/estimasi`: komposer+katalog+harga+rekap PPN hidup; layar Material/RAP belum ada)**. ~~**Langkah 6 (Material take-off/BBS/steel_profiles) dan 7 (RAP/Pagu) BELUM DIMULAI — 0 tabel di migration manapun**~~ → **KOREKSI 2026-08-16: klaim "0 tabel" SALAH/BASI.** Diukur ke migrasi DAN ke DB nyata: langkah 6 = `122_cecep_material_takeoff.sql` (tabel `material_pack`, `steel_profiles`), langkah 7 = `138_cecep_rap_pagu.sql` (tabel `rap_budget`, `rap_material_line`, `rap_labor_line`, `rap_change_log`). Semua hidup di DB dengan RLS aktif (`node scripts/db/introspect.mjs tables`), dan endpoint-nya menjawab 200 (`/cecep/steel-profiles`, `/projects/:id/rap`, `/estimate-versions/:id/{material,rebar}-takeoff`). **Sisa pekerjaan nyata = LAPIS UI, bukan DB/API.** Diukur 2026-08-16 lewat sesi ber-login: tab Material & RAP merender 0 tabel (halaman putih tanpa empty state), dan `projects/:id/scenarios` memulangkan `[]` untuk SELURUH 17 proyek — rantai skenario→versi→item tak pernah dimulai, sehingga RAP/Kas/Varians kosong secara struktural. Langkah 9 (dpp_factor split) sengaja ditunda (gerbang D10). |
...
|---|---|---|---|---|---|
| 1 | **Tuntaskan CECEP sampai siap-pakai** (build order 10 langkah: CI isolation → config → seed AHSP → endpoint hitung → take-off material → **RAP/pagu**) | Estimasi tetap Excel: tender lambat & rawan salah; **tanpa pagu, belanja material tak terkendali — titik bocor #1 MASIH TERBUKA** (langkah 6/7 belum dimulai) | Langkah 1/3/4/5 ✅ (CI isolation tuntas, seed 2.620 assemblies verified, endpoint hitung+golden hidup, PR #86–91). ~~**Langkah 6 (take-off material/BBS) & 7 (RAP/Pagu) — 0 tabel, BELUM DIMULAI**~~ → **KOREKSI 2026-08-16: DB + API keduanya SUDAH ADA** (migrasi 122 & 138, tabel hidup ber-RLS, endpoint 200 — lihat baris `CECEP/` di §2). Yang benar-benar tersisa = **lapis UI**: 4 dari 6 tab `/estimasi` merender 0 tabel, dan 0 skenario di seluruh 17 proyek | Besar | Phase 3 / Program C — **AKTIF, prioritas berikutnya = UI (rombak `/estimasi`), bukan DB/API** |
...
| 3 | **Register piutang: AR aging + jadwal pelepasan retensi + recoupment uang muka** — ✅ **SELESAI 2026-07-28** (diselipkan saat jeda gate CECEP, pola #2): halaman `/piutang` (spektrum umur piutang + register retensi + register DP) + `GET /finance/{ar-aging,retention-register,dp-register}` + potongan DP di invoice progres (migration 124, saldo = DP terbayar − dipotong, `lib/ar-register.ts` ber-test) + menu DB-driven (125). Detail: taksonomi §14–15 | ~~Retensi lupa ditagih; DP tak dipotong = bayar dobel; telat nagih~~ → register + guard hidup, dijaga test positif & negatif | Invoice/termin (ada) | Kecil–sedang | Perpanjangan Modul 2 |
