...
| ~~Integrasi penyusutan → GL~~ | ~~1~~ | ✅ **SELESAI 2026-09-15** — tabel `penyusutan_alat` + `journal_entry_id` hidup dan terisi (migrasi 211), constraint menolak jurnal setengah jadi. **R-001 sudah SELESAI** (`RATIFIKASI.md` §R-001; dan §3a dokumen ini sudah menyatakannya SALAH sejak 2026-08-07 — baris ini tertinggal), dan rantai penjurnalannya hidup: `lib/jurnal-penyusutan.ts` → `routes/v1/alat-operasional.ts` → `POST /api/v1/alat-operasional/penyusutan/jurnalkan` (izin `gl:manage`, bukan `assets:manage` — yang dihasilkan JURNAL, dan jurnal mengubah laporan keuangan), tombolnya di `/aset/operasional` + `/pm-portal/aset/[id]`. Penjurnalan DIPICU OPERATOR per periode, bukan terjadwal — disengaja. Ukur sendiri: `grep -n "penyusutan/jurnalkan" apps/api/src/routes/v1/alat-operasional.ts` |
| ~~Izin kerja (work permit)~~ | ~~1~~ | ✅ **SELESAI 2026-08-07** — pemutus WAJIB berbeda dari pengaju (constraint DB + permission `k3:permit:decide`). Izin `disetujui` yang jendelanya lewat ditandai TIDAK BERIZIN |
