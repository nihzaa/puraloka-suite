...
| **Finansial** | `kasbons`, `invoices`, `payments`, `cash_accounts`, `expense_reports` | 🔴 Tinggi | 4 (terakhir, setelah pola terbukti di 3 kelompok sebelumnya) — **MUST** dijadwalkan di jam operasional rendah (mis. akhir pekan/malam hari, mengikuti perlakuan yang sama dengan migrasi 1C di [04-risk-register.md § R7](04-risk-register.md)) — *(v1.1: sebelumnya tidak ada timing/maintenance-window untuk kelompok ini meski dinilai risiko tertinggi di seluruh Phase 1; Phase 1A Readiness Review menemukan R7 sudah dapat perlakuan ini untuk migrasi yang risikonya lebih rendah, kelompok Finansial tidak)* |
...
3. **Verifikasi:** Test otomatis ([06-test-strategy.md](06-test-strategy.md)) memastikan role kustom baru sekarang mendapat akses yang benar via policy baru. **Untuk kelompok Finansial khusus** — logika policy **MUST** melalui review kedua yang independen dari sesi/konteks yang menulis policy tersebut (sesi AI terpisah tanpa konteks penulisan, atau pembacaan manual founder baris-per-baris terhadap threat checklist tertulis) sebelum lanjut ke langkah 4 — **MUST NOT** hanya mengandalkan "test lulus" sebagai satu-satunya bukti benar, karena penulis policy dan penulis test adalah aktor yang sama (risiko blind-spot bersama) *(v1.1: item baru — Phase 1A Readiness Review menemukan migrasi paling berisiko di roadmap ini tidak punya reviewer independen; RLS bug tidak terlihat lewat API karena `service_role` bypass RLS, sehingga test yang self-authored adalah satu-satunya deteksi tanpa langkah ini)*.
4. **Hapus policy lama** — migration terpisah, **hanya setelah** langkah 3 lulus dan berjalan stabil beberapa hari di staging/production tanpa insiden. **Untuk kelompok Finansial khusus** — karena observability penuh ([08-observability-plan.md](08-observability-plan.md)) belum aktif di Sub-Fase 1A dan Postgres tidak mencatat keputusan RLS per-baris secara default, "berjalan stabil" **MUST** diverifikasi aktif (bukan pasif menunggu laporan) lewat query terjadwal manual yang membandingkan jumlah baris yang terlihat per role terhadap jumlah yang diharapkan, dijalankan harian selama masa observasi — *(v1.1: item baru — Phase 1A Readiness Review menemukan tidak ada mekanisme deteksi kegagalan RLS senyap sampai Sub-Fase 1D; tanpa langkah interim ini, kebocoran data atau lockout pada tabel finansial bisa berjalan tanpa terdeteksi)*.
