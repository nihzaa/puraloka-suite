...
**Alasan teknis:** Dua Epic di Program C — RFI dan Submittals ([01-capability-to-task-mapping.md § Capability 1](01-capability-to-task-mapping.md#capability-1--mengelola-anggaran--progres-proyek-domain-project-delivery-program-a--program-c)) — secara struktural butuh approval chain generik. Membangunnya sebelum Workflow Engine tersedia berarti implementasi hardcode approval **ketiga** (setelah kasbon dan change order lama) yang harus dimigrasikan lagi nanti — investasi kerja yang dibuang dua kali.

**Catatan penting:** Ini **bukan** dependency untuk *seluruh* Program C — Epic lain (QC Checklist, HSE Incident Report, Punch List) **tidak** butuh Workflow Engine dan bisa dimulai segera setelah Program A selesai, paralel dengan Program B. Lihat Bagian 4 (Parallel Strategy) untuk pemisahan ini.
...
| Program C — Epic non-Workflow (QC Checklist, HSE, Punch List) ‖ Program B | Setelah Program A selesai | Epic ini tidak bergantung Workflow Engine (lihat Bagian 2, B → C) — bisa mulai begitu Program A selesai, tidak perlu menunggu Program B |
