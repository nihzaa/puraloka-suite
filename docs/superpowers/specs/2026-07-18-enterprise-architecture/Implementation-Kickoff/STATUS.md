...
| 1A.2 | Epic 4 | RLS Synchronization | 🟢 **Expand selesai** (PR #4✅#5✅#6) · ⏳ contract gated | `has_permission()` (062) + harness + fix recursion (065/ADR-005). **Expand SEMUA 4 kelompok tabel ✅**: Referensi (063), Operasional (066), Field ops (067/068), Financial (069/070). Capability baru: progress:manage, workers:manage, finance:manage, cash:manage (admin+pm default, UI-configurable). 103 test hijau. ⏳ **Contract phase (hapus policy lama literal-role di production) = GATE: maintenance-window + independent review + PITR verified** — lihat [contract gate](contract-gate-epic-4.md) |
