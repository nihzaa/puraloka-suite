...
### Phase 3 — Construction Core Modules (termasuk CECEP)
...
**Cakupan utama — CECEP (Construction Estimation & Cost Engineering Platform):** Bukan lagi "kandidat BOQ/AHSP" sederhana — CECEP adalah Core Cost Intelligence Platform penuh, sudah selesai fase perencanaan arsitekturnya (12 fase, `docs/superpowers/specs/2026-07-18-enterprise-architecture/CECEP/32-cecep-roadmap-v2.md`, seluruhnya Derived & Frozen). Cakupan: AHSP (4 sumber: Nasional/Company/Project/Custom), Assembly Library, RAB/RAP Builder dengan Contingency/Risk Allowance eksplisit, Price Book & Productivity Library bertingkat, Cost Control berbasis RAP (menggantikan basis RAB yang selama ini dipakai `kurva-s.ts`), Cashflow Forecast, Historical Cost Intelligence (Company Intelligence Loop — setiap proyek selesai memperbaiki akurasi estimasi berikutnya), dan AI Estimation (Excel-first).
**Kenapa ini bukan gap Tier 2 biasa:** [00 — Product Positioning](00-vision-and-business-architecture.md#product-positioning) eksplisit menyebut "kedalaman modul EVM/Kurva-S + Gantt WBS" sebagai diferensiator vs Procore/Primavera/SAP. CECEP memperdalam diferensiator ini lebih jauh — cost engineering AHSP/RAP/Company Intelligence Loop adalah kapabilitas yang bahkan produk enterprise asing tidak punya native untuk konteks Indonesia. Ini investasi diferensiasi produk untuk visi L3 (SaaS kontraktor lain), bukan sekadar pemenuhan gap operasional Puraloka Persada semata.
**Prasyarat urutan (ditemukan saat audit CECEP, dicatat eksplisit):** CECEP's Approval Workflow dirancang merujuk RBAC/Workflow Engine generik (bukan membangun state-machine sendiri) — idealnya dikerjakan SETELAH atau BERSAMAAN Phase 2 (Workflow Engine), bukan sebelumnya, supaya tidak membangun implementasi approval keempat yang harus dimigrasi lagi. CECEP juga menambah logic finansial baru (RAP, Risk Allowance, Cost Control basis RAP) — butuh test coverage Phase 1 sebagai jaring pengaman, konsisten Risk #2 di atas.
**Cakupan tambahan (kandidat lain, diprioritaskan saat fase ini dimulai):** Quality Control checklist, HSE incident report dasar — Tier 2 lain yang lebih ringan dari CECEP.
**Working software di akhir fase:** CECEP Milestone 1-2 minimal (Cost Code/Resource Identity/Assembly/AHSP/Price Book/Productivity berjalan — lihat [Implementation Roadmap CECEP](CECEP/49-phase11-implementation-roadmap.md) untuk urutan detail 4 milestone), plus modul Tier 2 ringan lain sesuai kebutuhan operasional saat itu.
...
| 9 | CECEP (Phase 3) dibangun sebelum Phase 2 (Workflow Engine) selesai — Approval Workflow CECEP terpaksa jadi implementasi state-machine keempat berdiri sendiri (setelah kasbon/CO/procurement), harus dimigrasi ulang begitu Workflow Engine generik selesai — dua kali kerja untuk hasil yang sama | Teknis/Efisiensi | Menengah (tergoda mulai cepat karena planning arsitektur sudah selesai) | Menengah (technical debt, bukan kegagalan fungsional) | Prioritaskan Phase 2 selesai dulu, atau minimal paralel — bukan Phase 3 CECEP mendahului Phase 2 | [CECEP/51](CECEP/51-final-audit-and-main-roadmap-position.md#bagian-4--kapan-main-roadmap-ini-dikerjakan) |
