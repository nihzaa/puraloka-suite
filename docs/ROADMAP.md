...
| 24 | **Capability Tier-2** — RFI, Submittals, Punch List, QC, HSE | Blueprint 01-capability-to-task | 🟡 **Punch List SELESAI 2026-08-01** (migrasi 156). Sisanya belum. Dependency "butuh Workflow Engine" diverifikasi **lunas ke kode**, bukan dibaca dari dokumen: 4 modul memakai `evaluateEntityApproval`, 6 chain berisi 13 langkah, ada baris progress nyata. Urutan pengerjaan dipilih dari dependency yang paling ringan — Blueprint menandai Punch List "butuh Field Ops core (sudah ada)", sementara RFI & Submittals butuh approval chain. Punch List juga menghasilkan data yang QC/NCR nanti rujuk, bukan sebaliknya | Besar (cicil per-Epic) |
