...
| **P4** | Web Push | ⛔ **TEMUAN LEBIH BESAR daripada "belum diuji di HP": rantainya PUTUS DI TIGA TEMPAT.** `sendWebPush()` punya **nol sebutan di seluruh `src/`** — bukan nol pemanggil, nol SEBUTAN. `subscribeToPush()` di web juga nol pemanggil dari UI. Service worker ada tapi tak pernah menerima apa pun. Konsisten dengan data: **0 dari 23 user** punya `push_subscription`. Sistem notifikasi menulis `channel: 'push'` ke DB tanpa pernah benar-benar mengirim push — jadi seluruh notifikasi hari ini **in-app saja**. Menguji di HP tak akan membuktikan apa pun karena tak ada yang dikirim. **Yang harus dikerjakan lebih dulu:** sambungkan `createNotifications()` → `sendWebPushToUsers()`, dan panggil `subscribeToPush()` dari UI (mis. saat user membuka panel notifikasi). Baru sesudah itu HP fisik berguna |
| **P5** | **§9a buta terhadap util mati** | ✅ **Diperbaiki 2026-08-01.** `audit-jalur-hidup.mjs` hanya memindai `src/lib/`, sehingga `utils/webpush.ts` lolos — alat yang dibuat untuk menangkap "benar tapi mati" justru melewatkan contoh terbesarnya. Diperluas ke `utils/`, dan deteksinya diperbaiki agar tak menuduh palsu (versi pertama menuduh `uuid`, `golden-runner`, `tenant-map.generated` yang sebenarnya diimpor relatif `'./x.js'` di direktori yang sama). **Empat temuan nyata sekarang terlihat:** `utils/webpush.ts` · `lib/golden-boq-adapter.ts` (dipakai test golden saja) · `lib/golden-runner.ts` (idem) · **`lib/retention-calculation.ts` — dan yang ini paling serius: `finance.ts` menghitung retensi SENDIRI dari kolom, jadi ada dua sumber kebenaran untuk perhitungan uang** |
