...
| **P6** | **ADR-004 dilanggar di sisi WEB** | 🔄 **Ratchet dipasang, procurement diperbaiki (32 → 27).** ADR-004 menetapkan kode memeriksa CAPABILITY, bukan nama jabatan — sisi API patuh (`requirePermission` di mana-mana), sisi web **tidak**: 27 pemakaian `user?.role === "admin"` di 12 berkas. **Sudah menggigit hari ini, bukan teori:** role kustom `direktur` punya **7 permission procurement** tapi UI menyembunyikan tombolnya karena mengecek `role === "admin" || role === "pm"`. Orang berwenang penuh melihat halaman tanpa tombol, **tanpa pesan apa pun**. Dampaknya "kosmetik" karena API tetap menolak — tapi itu justru lebih buruk: tombol yang muncul lalu 403 setidaknya memberi tahu; yang tak pernah muncul tak memberi apa-apa. `procurement/page.tsx` diperbaiki dengan permission yang **API benar-benar tuntut**, diverifikasi satu per satu ke `requirePermission` di rutenya: approve MR → `procurement:mr:manage` (level 1 rantai approval), PO & GR → `procurement:po:manage`, stok & opname → `procurement:view`. Sisanya dicicil — memaksa nol sekarang berarti menebak, dan **permission yang salah lebih berbahaya daripada role yang usang: ia MEMBUKA akses, bukan menutup** |
