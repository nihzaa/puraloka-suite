...
| 15c | **`multitenant-t3-rollback.test.ts` rapuh terhadap urutan jalan** | Temuan 2026-08-01 | 🟡 **Terdata, BUKAN regresi.** Test ini me-`DROP` lalu membangun ulang `company_id` di 32 tabel untuk menguji rollback migrasi 127. Ia mengandaikan berjalan di schema uji terisolasi, tapi tak punya gerbang apa pun yang memastikannya — di dev ia menyentuh schema nyata dan gagal dengan `relation "assemblies" does not exist` begitu jalan sesudah test lain yang mengganti `search_path`. **Dibuktikan bukan dari perubahan hari ini**: di-`stash` seluruh pekerjaan #15 dan dijalankan pada HEAD bersih (`76ce329`) — tetap 1 failed. Sendirian ia 23/23 hijau. Perbaikannya: gerbang `skipIf` bila bukan schema uji, atau setup yang menegakkan schema-nya sendiri | Kecil |
