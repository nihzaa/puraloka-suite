...
**Diperbarui:** 2026-09-11 (rev-24: **gelombang 2a** — keuangan · pengadaan · kontrak jadi native; **12 native · 8 baris mati · 0 buntu** (dari 9/11). Layar Kontrak menemukan `GET /api/v1/kontrak` RUSAK berbulan-bulan (embed self-join dengan nama constraint, bukan kolom) — 24 test hijau sepanjang itu sebab semuanya menguji POST/PATCH, tak satu pun GET daftarnya. Diperbaiki + 2 test. ⚠ `jadwal` TIDAK dibangun: `/api/v1/jadwal` ternyata PENJADWAL OTOMASI, bukan jadwal proyek — layar yang jalan dan memperlihatkan hal keliru lebih mahal daripada layar yang tak ada. Penjaga CI **241 hijau · 0 MERAH**. Sisa: G2b (5 modul, butuh rute baru) + G2c (3 terbesar). Detail: JOURNAL 2026-09-11 (lanjutan 2). Sebelumnya rev-23: WebView dicabut.)
