...
**Diperbarui:** 2026-09-12 (rev-26: **TAYANG DI PRODUKSI** — 149 commit, `265afa75` (5 Sep) → `1fe48c09`. Kedelapan langkah `perbarui-vps.sh` hijau untuk PERTAMA kalinya: sebelumnya langkah 7 (`audit-situs-host-dilayani`) melempar stack trace di TIAP deploy sejak 4 Sep tanpa seorang pun melihatnya — keluarannya dipotong `tail`, dan skripnya tetap melapor "SELESAI". Sebabnya `pg` tak ada di VPS; kini dipasang di `/opt/puraloka-alat` lalu di-symlink. Ikut ketahuan: bit executable `perbarui-vps.sh` tak pernah ter-commit (100644), jadi deploy KEDUA ditolak Permission denied. Terverifikasi tayang: `/health` menyebut commit-nya, `/api/v1/risiko` & `/kurva-s` hidup, gudang menampilkan "Besi Hollow 40×40×2mm" bukan UUID. Mobile: **19 dari 20 modul native, nol WebView**. Detail: JOURNAL 2026-09-12. Sebelumnya rev-25: gelombang 2b.)
